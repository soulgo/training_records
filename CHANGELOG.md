...
- Bot 回执与失败理由改用安全中文标签（如“体重/体脂率/活动热量/睡眠时长/睡眠评分”），区分“备 AI 补全成功 / 仍不完整 / 关键字段冲突 / 备用识别未配置”，不输出健康数值、OCR 原文或 Prompt。
