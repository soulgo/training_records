...
| `maintenance:sync` | sync | 是 | 调用现有 `syncTrainingCore`，同步 archive、markdown、thoughts 到 core |
| `maintenance:sync -- --phase archive` | sync | 是 | 仅同步 archive 到 core，兼容 `backfill:core` |
| `maintenance:sync -- --phase markdown` | sync | 是 | 仅同步 Markdown 到 core，兼容 `import:markdown` 与 `reconcile:markdown` |
| `maintenance:sync -- --phase thoughts` | sync | 是 | 仅同步 thoughts 到 core，兼容 `backfill:thoughts` |
| `export:markdown` | export | 是 | 通过统一维护入口导出派生后的 `训练记录.md` |
...
- 会写数据的普通增量同步统一使用 `npm run maintenance:sync`。
...
- Markdown 导出继续使用 `npm run export:markdown`，但实际入口已统一到 `tools/training-maintenance.mjs export markdown`。
