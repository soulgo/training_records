...
- 训练数据的 canonical 结构是 `TrainingSnapshot`；线上事实源是 PostgreSQL `core.*`。Markdown、Telegram 识别结果最终都应归一到该结构或其日级子结构，但 Markdown 只作为备份或显式兼容输入。
...
- Telegram 图片批次更新某一天时，会先确保 `core.training_day` stub 存在，再 upsert 本批次包含的 measurement、activities、nutrition 或 sleep 明细，并从子表刷新当天汇总。
...
Telegram sleep 正常图片路径写入 `core.sleep` 并刷新 `core.training_day` 睡眠汇总，不直接写 `archive.training_sleep`。完整 archive snapshot 仍由 `build:data` 的归档链路负责。
...
| `source/_posts/*-telegram-thought-*.md` | `可对外` | DB -> Markdown 备份 | 随想内容备份 Markdown |
| `source/images/thoughts/YYYY/MM/*` | `可对外` | Telegram 随想图片 | 随想图片本地资源；正文引用存入 `core.thought` |
...
| 数据库暂时不可用 | 批次 `persistenceStatus: pending_replay` | 等数据库恢复后再次运行 `sync:telegram` 重放队列 |
| 随想数据库写入失败 | Telegram 回复“已记录，数据库待补偿” | 修复数据库后重放 pending；正文以 `core.thought` 为准 |
