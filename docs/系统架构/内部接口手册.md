...
- 数据库写入失败时，图片批次会写入 PostgreSQL `ingest.telegram_pending_batch` 等待重放，不写 Markdown 兜底。历史 `runtime/telegram-sync-pending.ndjson` 默认不再自动消费，仅在显式设置 `TELEGRAM_SYNC_REPLAY_LEGACY_NDJSON_PENDING=true` 时作为 legacy 队列重放。
...
精简 batch 字段包括 `kind`、`batchId`、`taskId`、`sourceType`、`sourceId`、`taskStatus`、`retryState`、`retryCount`、`messageIds`、`updateIds`、`status`、`partialFailure`、`archivedDate`、`postPath`、`thoughtWriteStatus`、`persistenceStatus`、`persistenceError`、`failureCategory`、`failureReason`、`failureDisposition`、`recognitionPendingStatus`、`recognitionPendingError`、`aiCallLogStatus`、`pendingReplay`、`sourceImageCount`、`recognizedImageCount`、`failedImageCount`、`recognitionAttemptKinds`、`recognitionErrors`、`syncStages`、`warnings`、`issues`、`reason`、`dateSources`、`dateConfidence`、`dateStages`；分析批次额外包含 `analysisReplyStatus`、`analysisReplyError`、`analysisReplyParts`、`analysisAttemptKind`、`analysisModel`、`analysisSnapshotSource`。若 `timingsMs` 存在，会作为毫秒整数表输出。
...
| `runtime/telegram-sync-pending.ndjson` | `legacy` | 旧共享同步管线 | 默认不自动消费；显式 `TELEGRAM_SYNC_REPLAY_LEGACY_NDJSON_PENDING=true` 时重放旧 batch |
| PostgreSQL `ingest.telegram_pending_batch` | `内部稳定` | 共享同步管线 | 数据库失败后的默认待重放 batch 队列 |
...
- `telegram-sync-pending.ndjson` 每行是 `{ batch, failedAt, error }`，仅保留 legacy 格式兼容。
- 同一个 `batchId` 再次失败会去重保留最新记录。
- 队列最多保留 1000 条。
- 默认 `sync:telegram` / `sync:feishu` 不再自动消费该文件；需要迁移或人工恢复旧队列时，先运行 `node tools/telegram-sync-fallback.mjs inspect` 只读检查，再显式设置 `TELEGRAM_SYNC_REPLAY_LEGACY_NDJSON_PENDING=true`。
- legacy replay 成功清理或改写队列前会生成同目录 `telegram-sync-pending.ndjson.backup-<UTC timestamp>`，成功后再从原文件移除已处理批次。
