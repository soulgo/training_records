...
| 命令 | 稳定性 | 实现入口 | 作用 | 主要输入 | 主要输出 |
| --- | --- | --- | --- | --- | --- |
| `npm run sync:telegram` | `可对外` | `tools/telegram-sync.mjs` | 拉取或处理 Telegram updates，识别图片，写数据库/目标日期 Markdown，处理随想和分析命令 | Telegram、AI、数据库环境变量；可选 GitHub repository_dispatch payload | JSON 同步报告、数据库记录、必要时更新的 `训练记录.md`、`source/_posts/*`、`runtime/telegram-sync-pending.ndjson` |
...
| `npm run sync:db` | `内部稳定` | `tools/sync-training-core.mjs` | 顺序执行 archive 回填、训练 Markdown 对账和随想 Markdown 回填；CI 部署推荐入口 | Markdown、`source/_posts/*`、数据库环境变量 | `{ status, archive, markdown, thoughts }`；单段失败返回 `deferred`，整体可为 `partial` |
...
- `sync:db` 在 CI 中复用单个数据库连接完成 archive 回填和 Markdown 对账；随想回填保持独立事务，避免和训练日整日替换耦合。
...
- `ready + stored` 成功时以数据库为主事实源，只在需要更新人工账本时对目标日期做 Markdown 增量合并，不默认全量覆盖 `训练记录.md`。
...
图片批次 `ready + stored` 时，`runTelegramSync()` 会把业务数据写入 PostgreSQL 增量路径；同步 Action 内不再重建完整数据库快照或部署站点。只有 fallback、pending replay 或需要保持人工账本可见的目标日期合并会改写 `训练记录.md`。结果中的 `timingsMs` 会累计 `recognition`、`persist`、`markdownRewrite`、`notify`、`total` 等阶段耗时，供 Action summary 定位慢点。
...
Telegram sleep 正常图片路径只按本批次 upsert `archive.training_sleep`，不触发整日 archive snapshot 重写。完整 archive snapshot 仍由 `build:data` 的归档链路负责。
...
| 随想数据库写入失败 | Telegram 回复“Markdown 已写入，数据库待补偿” | 保留 Markdown 结果，修复数据库后重放 pending |
