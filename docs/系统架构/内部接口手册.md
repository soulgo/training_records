...
图片批次 `ready + stored` 时，`runTelegramSync()` 会把业务数据写入 PostgreSQL 增量路径；只有 fallback、pending replay 或需要保持人工账本可见的目标日期合并会改写 `训练记录.md`。
