...
飞书当前稳定支持新建 `/thought` / `/随想`，并复用模块 token 规则。编辑、删除和移动命令仍以 Telegram 数字 message id 定位习惯为主；飞书随想编辑/删除/移动不作为当前主支持路径。
...
- Markdown 文章由 DB -> Markdown 备份 workflow 导出到 `source/_posts/YYYY-MM-DD-telegram-thought-<message_id>.md`。
...
- 修改高风险边界时，先补或更新 targeted tests，并在当前专题文档或 [第二轮重构优化方案](../历史归档/第二轮重构优化方案.md) 中记录背景。
