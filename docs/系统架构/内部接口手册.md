...
- 如果数据库源不可用或缺少可渲染数据，并且数据库配置存在，`generateTrainingData` 会回退到 Markdown 并向 `stderr` 写提示；`TRAINING_SNAPSHOT_STRICT_DATABASE=true` 时不会回退。
- 数据库快照读取先使用多连接并发查询；如果连接/查询失败，会重试一次单连接读取，降低页面构建因连接抖动回退 Markdown 的概率。
...
GitHub Actions 中的 `sync:telegram` 不再等待站点构建部署。若同步产生仓库文件变化，后续由 push 触发 deploy workflow；若只有数据库入库变化，Telegram Sync workflow 会异步 dispatch 对应 deploy workflow，并传入 `strict_database_snapshot=true`。
...
图片批次 `ready + stored` 时，`runTelegramSync()` 会把业务数据写入 PostgreSQL；同步 Action 内不再重建完整数据库快照或部署站点，只在需要保持人工账本可见时对目标日期做 Markdown 增量合并。结果中的 `timingsMs` 会累计 `recognition`、`persist`、`markdownRewrite`、`notify`、`total` 等阶段耗时，供 Action summary 定位慢点。
