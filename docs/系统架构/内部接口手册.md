...
- 训练日 core 写入保持整日替换语义：按目标日期删除 measurement/activity/meal 子表，再 upsert `core.training_day` 并批量插入子表。
...
精简 batch 字段包括 `kind`、`batchId`、`status`、`archivedDate`、`postPath`、`thoughtWriteStatus`、`persistenceStatus`、`persistenceError`、`failureCategory`、`failureReason`、`recognitionErrors`、`warnings`、`issues`、`reason`；分析批次额外包含 `analysisReplyStatus`、`analysisReplyError`、`analysisReplyParts`。
...
- Telegram 图片批次更新某一天时，会先读取现有 day，再只替换该批次包含的 measurement、activities、nutrition 或 sleep 部分。
