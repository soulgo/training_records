...
| 部署入口 | `wrangler.feishu.toml` 的 `main = "cloudflare/feishu-sync-dispatch-worker.mjs"` |
| 方法 | `POST` |
...
| `GITHUB_OWNER` | 否 | `soulgo` | GitHub 仓库 owner |
| `GITHUB_REPO` | 否 | `training_records` | GitHub 仓库名 |
| `GITHUB_DISPATCH_EVENT_TYPE` | 否 | `feishu_update` | GitHub dispatch 事件类型；dev Worker 可设为 `feishu_update_dev` |
...
| `FEISHU_ENCRYPT_KEY` | `可对外` | 无 | Feishu Cloudflare Worker | 飞书事件签名校验 |
| `FEISHU_VERIFICATION_TOKEN` | `可对外` | 无 | Feishu Cloudflare Worker | 飞书事件 token 校验 |
| `GITHUB_OWNER` | `可对外` | `soulgo` | Cloudflare Worker | 目标仓库 owner |
| `GITHUB_REPO` | `可对外` | `training_records` | Cloudflare Worker | 目标仓库名 |
| `GITHUB_DISPATCH_EVENT_TYPE` | `内部稳定` | `feishu_update` | Feishu Cloudflare Worker | 飞书 dispatch 类型 |
...
| `FEISHU_IMAGE_BUFFER` | `内部稳定` | 无 | Feishu Cloudflare Worker | Durable Object binding，不是普通字符串变量 |
...
| 飞书 dispatch 未触发 | Feishu Worker 返回 `missing_github_token` 或 GitHub 没有 `feishu_update` run | 检查 Worker `GITHUB_TOKEN`、`GITHUB_OWNER`、`GITHUB_REPO`、`GITHUB_DISPATCH_EVENT_TYPE` |
