...
| 命令 | 稳定性 | 实现入口 | 作用 | 主要输入 | 主要输出 |
| --- | --- | --- | --- | --- | --- |
| `npm run sync:telegram` | `可对外` | `tools/telegram-sync.mjs` | 拉取或处理 Telegram updates，识别图片，增量写数据库，处理随想和分析命令 | Telegram、AI、数据库环境变量；可选 GitHub repository_dispatch payload | JSON 同步报告、数据库记录、随想图片 artifact、`runtime/telegram-sync-pending.ndjson` |
