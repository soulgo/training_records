...
8. ready 图片批次优先通过 `tools/training-db-write.mjs` 在同一事务内先写 `ingest.*`，再增量 upsert 本批次的 `core.measurement`、`core.activity`、`core.meal`、`core.sleep`，最后用单条 CTE upsert 刷新当前 `archivedDate` 的 `core.training_day` 汇总；本批没有出现的 workout/nutrition 模块不会覆盖历史模块。
9. 正常 `ready + stored` 图片批次不写 `训练记录.md`；Markdown 由定时备份 workflow 从数据库导出。
10. PostgreSQL 写入失败时，待补偿记录追加到 `runtime/telegram-sync-pending.ndjson`。
11. `/thought` 和身体反馈批次写入 `core.thought`；Markdown 文章和图片引用由备份 workflow 派生。
12. `/analysis` 读取 `TrainingSnapshot`，调用 AI 生成回复，通过 Telegram `sendMessage` 回发；按文档和代码语义不写 Markdown、`source/_posts` 或 PostgreSQL。
13. 若仓库内容变化，workflow 提交 `训练记录.md`、`source/_posts`、`source/images`，后续由 push 触发 deploy workflow；若只有 DB 内容变化，会异步 dispatch 独立 deploy workflow 并启用严格数据库快照模式。
