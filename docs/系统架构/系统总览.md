...
除手工维护 Markdown 外，系统还支持 Telegram 自动同步：Telegram Bot update 可经 Cloudflare Worker 转发为 GitHub `repository_dispatch`，由 GitHub Actions 执行 `npm run sync:telegram`，调用 AI 识别锻炼、体脂秤、饮食和睡眠截图，或处理 `/thought`、`/analysis` 等命令，并优先写入 PostgreSQL。数据库不可用时，训练图片批次会回退写 `训练记录.md`，随想批次会保留 `source/_posts` 产物并追加待补偿队列。
...
8. ready 批次优先通过 `tools/training-db-write.mjs` 写入 `ingest.*` 与 `core.*`，睡眠截图会写入 `core.sleep` 并更新 `core.training_day` 睡眠汇总字段。
...
| `.github/actions/site-build/action.yml` | 被 workflow 调用 | setup Node、cache `db.json`、`npm ci`、可选 `sync:db`/tests、`npm run build`、可选 Pages deploy | `public/` artifact 和 Pages 部署 |
...
| `.github/workflows/deploy-cloudflare-pages-dev.yml` | push dev 站点相关路径、手动 | 设置 `DEV_TRAINING_DB_*`，调用 site-build 构建和测试，删除 `public/CNAME`，Wrangler 上传 Cloudflare Pages | Dev 在线预览 |
| `.github/workflows/telegram-sync.yml` | 手动、`训练记录.md` push、`repository_dispatch: telegram_update` | 非 dispatch 维护时运行 `sync:db`、`npm run sync:telegram`、可选 export、检测变更、test:fast、commit/push、site-build | 内容同步和 Pages 部署 |
...
5. **共享 site-build action**：统一 Node setup、Hexo `db.json` cache、依赖安装、`sync:db`、测试、构建和 Pages deploy 步骤。
