...
12. 若仓库内容变化，workflow 提交 `训练记录.md`、`source/_posts`、`source/images`，后续由 push 触发 deploy workflow；若只有 DB 入库变化，会异步 dispatch deploy workflow 并启用严格数据库快照模式。
...
| `.github/workflows/telegram-sync.yml` | 手动、`训练记录.md` push、`repository_dispatch: telegram_update` | 非 dispatch 维护时运行 `sync:db`、`npm run sync:telegram`、可选 export、检测变更、test:fast、commit/push、DB-only 时异步触发 deploy | 内容同步、结果通知和异步页面部署触发 |
...
3. **Telegram Sync**：Telegram update 或 `训练记录.md` push 后同步数据、提交内容变更；仓库变化交给 push deploy，DB-only 入库异步触发 deploy。
