...
| `.github/workflows/telegram-sync.yml` | 手动、`训练记录.md` push、`repository_dispatch: telegram_update` | 非 dispatch 维护时运行安全 `sync:db`；dispatch 快速路径透传 AI/recognition 变量并运行 `sync:telegram`、写 summary、检测文件/DB 变化、commit/push、DB-only 时异步触发 deploy | 内容同步、结果通知和异步页面部署触发 |
