...
| `.github/workflows/telegram-sync.yml` | 手动、`训练记录.md` push、`repository_dispatch: telegram_update` | 非 dispatch 维护时运行安全 `sync:db`；dispatch 快速路径运行 `sync:telegram`、写 summary、检测文件/DB 变化、commit/push、DB-only 时异步触发 deploy | 内容同步、结果通知和异步页面部署触发 |
...
| `TELEGRAM_SYNC_RUN_SLEEP_BACKFILL` | GitHub Actions/本地 sync | 显式运行睡眠全量补偿；默认在 pending replay 或新存储的睡眠图片后触发 |
