...
系统支持 Telegram 自动同步：Telegram Bot update 可经 Cloudflare Worker 转发为 GitHub `repository_dispatch`，由 GitHub Actions 执行 `npm run sync:telegram`，调用 AI 识别锻炼、体脂秤、饮食和睡眠截图，或处理 `/thought`、`/analysis` 等命令，并写入 PostgreSQL。正常图片成功路径会增量 upsert 本批次数据并刷新目标日期汇总，不整日替换同日其它模块；数据库不可用时追加待补偿队列。Markdown 由定时备份 workflow 从数据库导出。
...
12. `/thought` 和身体反馈批次写入 `core.thought`；Markdown 文章和图片引用由备份 workflow 派生。
