...
8. 回执为“缺少必要字段 / 备用识别未配置”时，先看是硬完整性还是条件完整性缺失（`src/core/ai/recognition-completeness.mjs`），再确认是否配置了 `TELEGRAM_RECOGNITION_FALLBACK_*` 三项；备用未配置时主识别缺字段会走 `fallback_unavailable`，仍会把图片上已有的数据入库，只是缺失字段无法补全。
...
- fallback provider 需要 API key、base URL、model 三项同时存在；未配置时不会创建备用 provider，主识别缺字段只能靠重发同一张更清晰的截图补全。
