...
飞书不是独立业务实现。`runFeishuSync` 加载飞书 API 配置后调用渠道无关的 `runMessageSync`（`src/app/use-cases/message-sync.use-case.mjs`），并覆盖 adapter：
...
飞书消息在 `groupFeishuUpdates` 内被转换为渠道无关消息形状，再调用共享的 `groupSourceMessages`。这保证图片、分析、随想新增/编辑/删除/移动走同一套业务语义。
