...
- **仅两类拒绝入库**：主备关键字段确定性冲突（`reconciliation.status = 'conflict'`，或 `normalizeActivities()` 抛出 `ACTIVITY_METRIC_CONFLICT`），或整张图无任何可写入数据。餐次建议范围以及睡眠阶段明细（深睡/浅睡/REM/清醒分钟与对应比例）冲突不阻断，保留主识别值继续写入。阻断时返回 `skipped` + `failureDisposition: 'manual_intervention'`、`failureCategory: 'business_incomplete'`，`core.*` 对该批次零写入，回执给出安全中文原因（如“主备识别的关键字段不一致（体重），需要人工核对”“主识别缺少必要字段（…），备用识别未配置”）。
