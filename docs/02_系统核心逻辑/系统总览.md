...
- webhook 请求不读取 Telegram polling offset；pending 只由 `SYNC_REPLAY_MODE=scheduled` 的独立 workflow 按 dev/main 与 Telegram/飞书消费，新消息不替历史失败任务还债。
...
| dev/main pending 重放 | `Pending Replay` 定时或手工触发 |
| 站点构建 | `npm run build` |
