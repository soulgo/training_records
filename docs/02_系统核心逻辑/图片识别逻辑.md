...
识别场景中主 provider 使用通用 `AI_API_PROTOCOL`；fallback 默认继承该协议，只有备用服务协议不同才通过 `TELEGRAM_RECOGNITION_FALLBACK_API_PROTOCOL` 覆盖。`createRecognitionAiProvider` 会用 `TELEGRAM_RECOGNITION_MODEL` 覆盖通用 `AI_MODEL`。配置 `TELEGRAM_RECOGNITION_FALLBACK_MODEL` 即启用 fallback；备用 key/base URL 按 `STANDBY_AI_*`、旧 `TELEGRAM_RECOGNITION_FALLBACK_*`、主 `AI_*` 的顺序取首个非空值。`AI_BASE_URL` / `STANDBY_AI_BASE_URL` 必须是 API base（通常到 `/v1`），由 provider 依协议追加 `/responses` 或 `/chat/completions`。Kimi 与 GPT 来自不同服务或路由分组时必须配置 `STANDBY_AI_API_KEY`、`STANDBY_AI_BASE_URL`，否则 Kimi 会被错误发送到主 GPT endpoint。
