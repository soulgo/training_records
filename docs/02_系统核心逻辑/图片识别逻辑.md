...
flowchart TD
  ...
  S -->|服务失败| W[fallback_failed：沿用主结果并安全警告]
...
识别场景中主 provider 使用通用 `AI_API_PROTOCOL`；fallback 默认继承该协议，只有备用服务协议不同才通过 `TELEGRAM_RECOGNITION_FALLBACK_API_PROTOCOL` 覆盖。`sync.yml`、`sync-dev.yml` 和 `pending-replay.yml` 均显式注入该覆盖变量，实时同步与 pending replay 不会出现协议漂移。`createRecognitionAiProvider` 会用 `TELEGRAM_RECOGNITION_MODEL` 覆盖通用 `AI_MODEL`。配置 `TELEGRAM_RECOGNITION_FALLBACK_MODEL` 即启用 fallback；备用 key/base URL 按 `STANDBY_AI_*`、旧 `TELEGRAM_RECOGNITION_FALLBACK_*`、主 `AI_*` 的顺序取首个非空值。`AI_BASE_URL` / `STANDBY_AI_BASE_URL` 必须是 API base（通常到 `/v1`），由 provider 依协议追加 `/responses` 或 `/chat/completions`。Kimi 与 GPT 来自不同服务或路由分组时必须配置 `STANDBY_AI_API_KEY`、`STANDBY_AI_BASE_URL`，否则 Kimi 会被错误发送到主 GPT endpoint。

触发 fallback 的错误来自 `shouldRetryWithFallbackProvider`：`AiProviderError`、HTTP 404/429/5xx、Abort/Timeout、HTTP 200 HTML 等造成的原生 JSON `SyntaxError`、以及包含 timeout/rate limit/network/fetch failed 等文本的错误。404 只触发切换备用连接，不会在同一错误 endpoint 做重试。这是**技术 fallback**，与下述**业务完整性 fallback** 是两条独立分支，不互相伪装。

备用 provider 的 `requestChatCompletion` 会被串行化（`serializeAiProviderRequests`，`src/app/use-cases/message-sync.use-case.mjs`），同一时刻最多一个在途请求。原因是备用服务（如 Kimi）通常有更严格的组织并发上限（`organization concurrency: 1`）：主 AI 失败后同一相册多张图会并发切换到备用 provider，第二个请求立即被 429 拒绝且退避窗口远短于备用请求的实际耗时。串行化让备用请求排队依次执行，主 provider 保持原有 `AI_CONCURRENCY` 并发不受影响。
...
6. 业务补全 provider 服务失败 → 结果标 `reconciliation.status = 'fallback_failed'`，加入安全警告并沿用主结果；现有可写入数据继续入库。主 AI 技术失败后的备用 AI 也失败仍按技术失败处理。
...
- 关键数值字段超出容差 → 记 `conflictFields`、状态 `conflict`，不按模型置信度自动二选一；餐次建议范围以及睡眠阶段明细（深睡/浅睡/REM/清醒分钟与对应比例）属于非关键字段，冲突不阻断，保留主识别值继续入库。
