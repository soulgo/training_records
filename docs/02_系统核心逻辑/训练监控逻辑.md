...
| 数据生成入口 | `tools/generate-training-data.mjs:24` |
| 注册 MonitorGenerator | `tools/generate-training-data.mjs:80` |
| 监控视图输出路径 | `tools/generate-training-data.mjs:40,91` |
...
| 工具兼容入口（re-export） | `tools/monitor-view.mjs:1` |
...
- 视图模型核心逻辑在 `src/site/monitor-view.mjs`，`tools/monitor-view.mjs` 仅 re-export 供测试与脚本引用，改动应落在源文件。
