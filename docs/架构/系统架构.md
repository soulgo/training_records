...
随想图片写入边界位于 `tools/telegram-thoughts.mjs` 的 `ImageStorageAdapter`。`COS_ENABLED` 未启用时使用 `github_local` provider，写入本地 `source/images/thoughts`；启用 `COS_ENABLED=true` 且 `COS_PROVIDER=tencent_cos` 时，GitHub Actions 用腾讯云 COS SDK 上传图片，并把完整公有读 URL 交给 `core.thought.image_refs_json`。
