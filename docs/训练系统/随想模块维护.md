...
三个模块共用同一套消息通道写入、带图、pending queue 和 DB 镜像逻辑，只在命令解析和页面过滤时按模块分流。Telegram 当前支持新建、编辑、删除、移动和 Markdown 文档附件；飞书当前支持新建 `/随想`、图片随想 artifact 和 DB-first 入库，编辑/删除/移动后续需要在多通道身份模型收敛后再扩展。
...
Markdown 文档附件不会新增数据库字段；附件内容会在持久化前写入 `body`，后续仍由 DB -> Markdown 导出生成 `source/_posts/*telegram-thought-*.md` 或 `source/_posts/*feishu-thought-*.md` 并交给 Hexo 渲染。
