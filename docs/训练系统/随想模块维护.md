...
- 文档附件 caption 也走同一套解析；当附件是 Markdown 文档时，caption 只负责命令和模块，正文优先来自附件内容
...
Markdown 文档附件不会新增数据库字段；附件内容会在持久化前写入 `body`，后续仍由 DB -> Markdown 导出生成 `source/_posts/*telegram-thought-*.md` 并交给 Hexo 渲染。
