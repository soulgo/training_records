...
| `sql/pgsql17.sql` | 完整初始化脚本、`training_writer` 角色、grant、索引和 schema 基准。 |
...
| 优先级 | 文档 | 为什么排在这里 |
| --- | --- | --- |
| 高 | [01_高优先级_权限收敛与运行时DDL下线.md](01_高优先级_权限收敛与运行时DDL下线.md) | 直接影响数据库安全边界。当前业务写入前会执行 DDL，初始化脚本中的 `training_writer` 对 `archive/ingest/core` 权限也偏宽。 |
...
- [ ] main/dev 配置仍与 `docs/01_系统配置/` 一致，Secret 不写入文档和日志。
- [ ] `npm run maintenance:inspect` 能只读查看 pending、AI monitoring 和数据库可用性。
- [ ] `npm run sync:db -- --dry-run`、`npm run maintenance:migrate -- --dry-run` 能输出可审计计划。
- [ ] `npm run check:data-consistency` 作为每阶段数据库变更后的固定验收命令。
- [ ] Action summary 中 DB 字段仍只包含安全摘要，不出现 DB URL、SQL 参数、用户文本或 token。
