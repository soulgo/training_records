...
> 状态：高优先级“权限收敛与运行时 DDL 下线”已实现并移入重构历史；本目录只保留仍未实现的中 / 低优先级规划。
...
| 原规划 | 当前位置 | 当前事实入口 |
| --- | --- | --- |
| `01_高优先级_权限收敛与运行时DDL下线.md` | [重构历史/数据库优化/01_高优先级_权限收敛与运行时DDL下线.md](../../重构历史/数据库优化/01_高优先级_权限收敛与运行时DDL下线.md) | [系统配置](../../../01_系统配置/README.md)、[数据库模型](../../../02_系统核心逻辑/数据库模型.md)、[数据入库流程](../../../02_系统核心逻辑/数据入库流程.md)、[PostgreSQL 排查](../../../04_问题与排查/PostgreSQL.md) |

## 总体路线

1. 中优先级：统一连接 factory/pool，下推日期范围查询，为读取链路补慢查询观测，降低连接峰值和全量读取放大。
2. 低优先级：让站点构建直接消费 PostgreSQL，Markdown 只保留为导出备份和显式人工恢复入口。
...
- [ ] `npm run check:data-consistency` 作为每阶段数据库变更后的固定验收命令；命令已优先使用 `TRAINING_DB_READONLY_URL`，每次中 / 低优先级落地后仍需在目标环境执行通过。
