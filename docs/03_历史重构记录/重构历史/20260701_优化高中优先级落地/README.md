# 20260701 优化高 / 中优先级落地记录

> 状态：高 / 中优先级已实现并移入重构历史；低优先级剩余项见 [后续规划_未实现/20260701_优化](../../后续规划_未实现/20260701_优化/README.md)。
> 来源：2026-07-01 dev/main GitHub Actions 截图、`gh run view` 日志抽样、当前代码和系统核心文档复核。
> 当前事实入口：`docs/02_系统核心逻辑/`、`docs/01_系统配置/`、`docs/04_问题与排查/`。本目录只用于追溯当时规划和验收，不作为当前操作入口。
...
- 不把 `core.*`、`ingest.*`、`archive.*` 的事实源关系重新设计；数据库权限和连接池优化仍以既有 [数据库优化](../../后续规划_未实现/数据库优化/README.md) 为主线。
