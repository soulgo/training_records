...
| `参数有效时间监控/` | 2026-07-07 | 系统参数有效期 registry、audit 和页面展示 | 已从“有效期推断”修正为 `config/parameter-health/<env>.json`、`Parameter Health Audit`、`monitor.system_config_parameters/checks` 和 `/action-monitor/` 参数健康模块；当前事实见 `01_系统配置/`、`02_系统核心逻辑/Action日志与失败补偿.md`、`02_系统核心逻辑/数据库模型.md` 与 `04_问题与排查/Action日志.md` |
...
| `20260711_整体重构优化/` | 2026-07-11 | 全仓入口、facade、同步合同和过渡兼容收敛 | 已删除浅层架构壳与纯转发模块，统一 `batches` 同步合同，飞书直达共享消息管线，下线运行时 schema preflight 与 NDJSON pending，并同步 SQL 和当前文档 |
