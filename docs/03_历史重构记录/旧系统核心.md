...
> 历史归档：本文是迁移前的系统核心快照，不是当前事实入口。当前逻辑请阅读 [系统核心逻辑](../02_系统核心逻辑/README.md)。

本文记录当时的系统核心逻辑，覆盖系统是什么、架构、业务能力、消息链路、AI 识别、数据模型、运维排障、开发指南和命令接口参考。当时的部署配置见 [旧系统配置](旧系统配置.md)；历史重构见 [重构历史](重构历史/README.md)。
