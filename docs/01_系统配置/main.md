...
| `TELEGRAM_RECOGNITION_FALLBACK_API_KEY` | 可选 | 图片识别备用 AI provider 的 key；不填继承 `AI_API_KEY`，仅备用服务商不同时需要单独配置。 |
| `TELEGRAM_RECOGNITION_FALLBACK_BASE_URL` | 可选 | 备用 AI provider base URL；不填继承 `AI_BASE_URL`。 |
