...
参数有效期元数据维护在 `config/parameter-validity/dev.json`。当前 registry 只登记第一批高风险 Secret、变量和运行时参数的名称、范围、分类、来源、有效期或复核日期，不保存实际值或 value hash；没有填写 `expiresAt`、`reviewAfterAt`，且无法通过 `rotationCycleDays` 计算到期日的参数会在 `/action-monitor/` 中显示为 `unknown`。
