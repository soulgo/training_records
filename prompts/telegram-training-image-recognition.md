<!-- prompt-metadata {"version":"2026-08-03","schemaName":"telegram_training_image","schemaVersion":"v4","sourceVersions":{"shared":"2026-06-01","recognition":"2026-08-03","appProfiles":"2026-06-13"}} -->
...
- `detail`：保留活动的简短可读说明，不作为结构化指标的来源
- `durationSeconds`：活动时长秒数；不可见时填 `null`
- `calories`：活动消耗 kcal；不可见时填 `null`
- `heartRate`：平均或页面明确标注的活动心率；不可见时填 `null`
- `distanceKm`：活动距离公里数；不可见时填 `null`
- `avgSpeedKmh`：平均速度 km/h；不可见时填 `null`
...
- `calories`：该餐或该项热量，单位 kcal；必须输出 JSON number，例如 `510`，不要输出 `"510 kcal"` 或带单位文本
- `recommendedMin`、`recommendedMax`：建议范围；有可靠范围时输出 JSON number，没有可靠范围时填 `null`，不要输出带单位文本
...
睡眠阶段、阶段占比和睡眠健康指标必须写入同一个 `records.sleep`；不要拆成第二条睡眠记录，不要只写总时长而漏掉画面可见的阶段字段。
...
睡眠页面只有单个醒来页头日期（例如 `8/3` 或 `8月3日`）且无年份时，用消息年份补全为 `YYYY-MM-DD`；该日期仍必须来自截图，程序会按睡眠逻辑归档到前一日。
