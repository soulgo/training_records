...
- `no_recent30`：主结论只使用 recent7、measurementTrend7 和 latestDays。不要引用 recent30 或 measurementTrend30 作为结论；若必须提及，只能标注为长期背景。
- `recent7_supplement`：recent7 只能作为近期变化补充，不要替代30天主结论。
- `explicit_mixed`：可以对比 recent7 和 recent30，但每个数字都必须标注对应时间窗。
- `near_term`：以最近7天负荷和最近5天细节为主。recent30 只能作为长期趋势背景，不要主动展开。
- `default_recent7`：默认以最近7天给可执行建议。recent30 只能作为长期趋势背景；如引用必须明确说「30天背景」。
